...
beforeEach(async () => {
    ...
    let implementationInstance = await new web3.eth.Contract(compiledQuestion)
    .deploy({
        data: compiledQuestion.bytecode,
    })
    .send({ from: admin, gas: maxGas});

    let implementation = implementationInstance.methods;
